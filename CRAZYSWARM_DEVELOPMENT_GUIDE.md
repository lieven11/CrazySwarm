...
- Configured rooms, geofences, axis-aligned obstacles, range rays, IMU, optical
  flow, modeled transport latency/loss, and injected failures.
...
Its physical coefficients are `CONFIGURED_UNQUALIFIED`. It does not currently
model propeller RPM, aerodynamic wash, ground effect, detailed contact/crash
response, camera imagery, or physical RF behavior, and it does not execute the
actual Crazyflie firmware control loop. It remains useful and must not be deleted
when Isaac Sim is added.
...
- [ ] Current simulator passes the Fast Simulator work-packet gate with a clean test, type, lint, UI, and real-time verification baseline.
- [ ] One immutable mission artifact executes through both the current and mocked Isaac adapters under the same contract.
...
1. Execute `WorkPackets/FAST_SIMULATOR_WP.txt` and close the interrupted real-time
   hover verification, strict-typing failures, UI visual/load verification, and
   simulator contract gaps.
2. Execute `WorkPackets/NVIDIA_ISAAC_SIM_WP.txt` in order, beginning with the
   shared contract and mock gateway before downloading the NVIDIA runtime.
3. Select an RTX Linux/Windows host only after the installation packet's hardware,
   driver, storage, networking, and compatibility checklist is accepted.
4. Complete the full pre-hardware simulation gate before purchasing or connecting
   a real Crazyflie for project-controlled flight.
5. Before later hardware purchase, re-verify the exact drone, decks, battery,
   charger, radio, localization system, firmware, and safety procedure.
6. Keep the persistent-coverage demonstrator as the product milestone and treat
   formation flight as optional experimentation.
