...
| Last feedback reconciliation | 2026-08-12 |
...
| `REQ-EVI-007` | Run history is a chronological investigation journal for a mission case. Ordinary reruns, changed locked inputs, changed planner/configuration hashes, comments, failures, and analysis findings remain in the active journal; none alone creates an implementation iteration or moves a run out of it. Only after the operator requests work packets from the retained findings, those packets complete the required implementation and independent-verification gates, and their changes are applied may an explicit archival transition start a new current implementation group. That transition must retain the case ID, implemented packet IDs, acceptance/verifier evidence, before/after implementation identities, and transition time. Earlier runs remain accessible below a clearly labeled `Previous implementation` divider; they are never deleted or silently mixed with current evidence. | Keeps iterative analysis intact until a verified implementation actually changes the product, while preserving a trustworthy comparison boundary afterwards. |
