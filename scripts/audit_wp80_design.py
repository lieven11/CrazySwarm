...
def ui_imports(path: str) -> set[str]:
    ...
    specifiers = re.findall(r"(?:from\s+|import\s*\()\s*[\"']([^\"']+)[\"']", text)
    return {
        resolved
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        default=ROOT / "missions/campaigns/sim/qualification/wp80-design-audit-v1.json",
    )
    ...
    if len(payload) != data["payload_bytes"] or sha256_bytes(payload) != data["payload_sha256"]:
        errors.append("WP-80 payload identity mismatch")
